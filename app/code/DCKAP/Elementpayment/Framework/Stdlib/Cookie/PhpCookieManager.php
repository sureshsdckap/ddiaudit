...
class PhpCookieManager extends \Magento\Framework\Stdlib\Cookie\PhpCookieManager
{
    ...
    protected function setCookie($name, $value, array $metadataArray)
    {
        ...
        if($name == 'PHPSESSID'){
            $phpSetcookieSuccess = setcookie(
                ...
                $value,
                $expire,
                $this->extractValue(CookieMetadata::KEY_PATH, $metadataArray, '').'; samesite=None',
        ...
        if (!$phpSetcookieSuccess) {
            $params['name'] = $name;
            if ($value == '') {
                throw new FailureToSendException(__('The cookie with "%1" cookieName couldn\'t be deleted.', $params['name'])
                    //new Phrase('The cookie with "%name" cookieName couldn\'t be deleted.', $params)
                );
            } else {
                throw new FailureToSendException(__('The cookie with "%1" cookieName couldn\'t be sent. Please try again later.', $params['name'])
    ...
    {
        if ($name == '' || preg_match("/[=,; \t\r\n\013\014]/", $name)) {
            throw new InputException(__('Cookie name cannot be empty and cannot contain these characters: =,; \\t\\r\\n\\013\\014')
//                new Phrase(
...
//                )
        ...
        if ($numCookies > static::MAX_NUM_COOKIES) {
            $this->logger->warning(__('Unable to send the cookie. Maximum number of cookies would be exceeded.'),
        ...
        if ($sizeOfCookie > static::MAX_COOKIE_SIZE) {
            throw new CookieSizeLimitReachedException(__('Unable to send the cookie. Size of \'%1\' is %2 bytes.', $name, $sizeOfCookie)
//                new Phrase(
