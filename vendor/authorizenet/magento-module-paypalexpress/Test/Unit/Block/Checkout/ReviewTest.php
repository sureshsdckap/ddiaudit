...
namespace AuthorizeNet\PayPalExpress\Test\Unit\Block\Checkout;

use PHPUnit\Framework\TestCase;
use \AuthorizeNet\PayPalExpress\Block\Checkout\Review;
...
class ReviewTest extends TestCase
{
     ...
     * @return \PHPUnit_Framework_MockObject_MockObject|\Magento\Quote\Model\Quote\Address\Rate
     */
    ...
    public function testGetPlaceOrderUrl()
    {
        ...
        $url = 'https://example.org/anet_paypal_express/checkout/place';

        $this->urlBuilderMock->expects(static::once())
            ->method('getUrl')
            ->with('anet_paypal_express/checkout/place', ['_secure' => true])
            ->willReturn($url);
    ...
    {
        $url = 'https://example.org/anet_paypal_express/checkout/saveShippingMethod';

        $this->urlBuilderMock->expects(static::once())
            ->method('getUrl')
            ->with('anet_paypal_express/checkout/saveShippingMethod', ['_secure' => true])
            ->willReturn($url);
    ...
    public function testGetPayPalAccount()
    {
        ...
        $this->quoteMock->expects(static::any())->method('getPayment')->willReturn($paymentMock);

        $paymentMock->expects(static::once())
            ->method('getAdditionalInformation')
            ->with(\AuthorizeNet\PayPalExpress\Model\Checkout::KEY_PAYER_EMAIL)
            ->willReturn($payerEmail);

        static::assertEquals($payerEmail, $this->reviewBlock->getPayPalAccount(), 'payer email incorrect');
    }
}
